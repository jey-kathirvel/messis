"""Create the PATCH-IRR-007 PostgreSQL backup without printing credentials."""
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: backup_patch_irr_007.py OUTPUT.dump")
    pg_dump = shutil.which("pg_dump")
    if not pg_dump:
        raise SystemExit("pg_dump is required before PATCH-IRR-007 deployment")
    ...
    if not output.is_file() or output.stat().st_size == 0:
        raise SystemExit("database backup was not created")
