"""Create the PATCH-IRR-006 PostgreSQL backup without printing credentials."""
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("usage: backup_patch_irr_006.py OUTPUT.dump")
    pg_dump = shutil.which("pg_dump")
    if not pg_dump:
        raise SystemExit("pg_dump is required before PATCH-IRR-006 deployment")
